...
function Calendar({
  ...
  const defaultComponents = {
    // `orientation` is a DayPicker-internal prop; strip it so it isn't spread
    // onto the underlying SVG DOM node (which would trigger a React warning).
    Chevron: ({ orientation, ...chevronProps }: { orientation?: string } & Record<string, unknown>) => {
      if (orientation === 'left') {
        return <ChevronLeft size={16} strokeWidth={2} aria-hidden="true" {...chevronProps} />;
      }
      return <ChevronRight size={16} strokeWidth={2} aria-hidden="true" {...chevronProps} />;
    },
  };
